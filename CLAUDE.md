...
Three scripts. Together they are the complete one-command installation flow.

---

**`setup.sh`** — The single entry point. Users run this after cloning the repo
and filling in `.env`. It handles everything automatically.
...
for i in $(seq 1 30); do
  ...
  [ $i -eq 30 ] && echo -e "${RED}Timeout. Check: docker compose logs${NC}" && exit 1
...
for i in $(seq 1 72); do
  ...
  [ $i -eq 72 ] && echo -e "${RED}Timeout. Check: docker compose logs llm${NC}" && exit 1
...
**`setup/init-company.sh`** — Called by setup.sh. Creates company and CEO in Paperclip.
...
- Print: `[TASK 5 COMPLETE] — setup.sh, init-company.sh, activate-phase.sh written`
...
cp .env.example .env
nano .env   # or your editor of choice — fill in all required values
...
- Every `instructionsFilePath` in the codebase resolves to a file that exists under `instructions/`
- `setup.sh` has `chmod +x` — is executable
- Check that `./instructions` is mounted read-only in `docker-compose.yml`
  under the `paperclip` service so agents can read instructions files at runtime
- Verify the `adapter/` directory will be correctly picked up by Paperclip's
  adapter registry (check the register call in Task 4)
- Print a final directory summary:
  ...
  Contents:
    ...
    migrations/     — [N] SQL migrations across 3 phases
    adapter/        — custom local_llm adapter
    staff-ui/       — React staff dashboard
